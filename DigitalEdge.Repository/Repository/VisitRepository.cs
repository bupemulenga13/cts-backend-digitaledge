    ...
    public class VisitRepository : IVisitRepository
    {
        ...
        public List<AppointmentsModel> GetAppointmentsDetails()
        {
            List<AppointmentsModel> appointmentsdetails = (from appointment in _DigitalEdgeContext.Appointments
                join visit in _DigitalEdgeContext.Visits on appointment.AppointmentId equals visit.AppointmentId into
                    appointments
        ...
        public List<AppointmentsModel> GetUpcomingAppointment(VisitsModel upcommingfilterdata)
        {
            List<AppointmentsModel> upcommingappointmentsfilter = (from appointment in _DigitalEdgeContext.Appointments
                ...
                join client in _DigitalEdgeContext.Clients on appointment.ClientId equals client.ClientId into list
                from clients in list.DefaultIfEmpty()
                where appointment.ServiceTypeId == upcommingfilterdata.ServiceTypeId &&
        ...
        public List<AppointmentsModel> GetAppointmentsMissedFilter(VisitsModel missedfilter)
        {
            List<AppointmentsModel> appointmentsmissedfilter = (from appointment in _DigitalEdgeContext.Appointments
                ...
                join client in _DigitalEdgeContext.Clients on appointment.ClientId equals client.ClientId into list
                from clients in list.DefaultIfEmpty()
                where appointment.ServiceTypeId == missedfilter.ServiceTypeId &&
                      appointment.AppointmentDate < DateTime.UtcNow
                select new AppointmentsModel
                {
                    Id = appointment.AppointmentId,
        ...
        public List<ClientModel> GetClientDetails(string searchTerm)
        {
            List<ClientModel> viewClientdetails = (from client in _DigitalEdgeContext.Clients
                where client.FirstName.Contains(searchTerm) || client.LastName.Contains(searchTerm) ||
                      client.ArtNo.Contains(searchTerm)
        ...
        public List<ClientModel> GetClients()
        {
            List<ClientModel> clients = (from client in _DigitalEdgeContext.Clients
                    ...
                    join status in _DigitalEdgeContext.ClientStatuses on client.ClientStatusId equals status
                        .ClientStatusId
        ...
        public List<ClientModel> GetClientsByFacility(long facilityId)
        {
            List<ClientModel> clientsInFacility = (from clients in _DigitalEdgeContext.Clients
                    ...
                    join status in _DigitalEdgeContext.ClientStatuses on clients.ClientStatusId equals status
                        .ClientStatusId
        ...
        public List<AppointmentsModel> GetAppointments()
        {
            List<AppointmentsModel> appointments = (from appointment in _DigitalEdgeContext.Appointments
                    join client in _DigitalEdgeContext.Clients on appointment.ClientId equals client.ClientId into
                        clientAppt
                    ...
                    join department in _DigitalEdgeContext.ServiceTypes on appointment.ServiceTypeId equals department
                        .ServiceTypeId into apptDepartment
                    ...
                    select new AppointmentsModel
                    {
                        ...
                        {
                            FirstName = clientAppts.FirstName, LastName = clientAppts.LastName,
                            ArtNo = clientAppts.ArtNo, ClientPhoneNo = clientAppts.ClientPhoneNo
                        },
        ...
        public List<AppointmentsModel> GetClientDetailsFilters(VisitsModel filterdata)
        {
            ...
            if (filterdata.FacilityId > 0 && filterdata.ServicePointId == 0 && filterdata.ProvinceId == 0 &&
                filterdata.DistrictId == 0)
            {
                viewClientdetailsfilter = (from appointment in _DigitalEdgeContext.Appointments
                    join visit in _DigitalEdgeContext.Visits on appointment.AppointmentId equals visit.AppointmentId
                        into appointments
                    from visits in appointments.DefaultIfEmpty()
                    join client in _DigitalEdgeContext.Clients on appointment.ClientId equals client.ClientId into list
                    from clients in list.DefaultIfEmpty()
            ...
            else if (filterdata.FacilityId > 0 && filterdata.ServicePointId > 0 && filterdata.ProvinceId == 0 &&
                     filterdata.DistrictId == 0)
            {
                viewClientdetailsfilter = (from appointment in _DigitalEdgeContext.Appointments
                    join visit in _DigitalEdgeContext.Visits on appointment.AppointmentId equals visit.AppointmentId
                    ...
                    from clients in list.DefaultIfEmpty()
                    where visits.FacilityId == filterdata.FacilityId &&
                          visits.ServicePointId == filterdata.ServicePointId
                    select new AppointmentsModel
                    {
                        Id = appointment.AppointmentId,
                        ClientId = clients.ClientId,
                     ...
                     filterdata.DistrictId == 0)
            {
                viewClientdetailsfilter = (from appointment in _DigitalEdgeContext.Appointments
                    ...
                    join client in _DigitalEdgeContext.Clients on appointment.ClientId equals client.ClientId into list
                    from clients in list.DefaultIfEmpty()
                    join facility in _DigitalEdgeContext.Facilities on visits.FacilityId equals facility.FacilityId into
                        facilitylist
                    from facilitys in facilitylist.DefaultIfEmpty()
                    join district in _DigitalEdgeContext.Districts on facilitys.DistrictId equals district.DistrictId
                        into districtlist
                    from districts in districtlist.DefaultIfEmpty()
                    join province in _DigitalEdgeContext.Provinces on districts.ProvinceId equals province.ProvinceId
                        into provincelist
                    from provinces in provincelist.DefaultIfEmpty()
                    where visits.FacilityId == filterdata.FacilityId &&
                          visits.ServicePointId == filterdata.ServicePointId &&
                          provinces.ProvinceId == filterdata.ProvinceId
                    select new AppointmentsModel
                    {
                        Id = appointment.AppointmentId,
                        ClientId = clients.ClientId,
                     ...
                     filterdata.DistrictId > 0)
            {
                viewClientdetailsfilter = (from appointment in _DigitalEdgeContext.Appointments
                    ...
                    join province in _DigitalEdgeContext.Provinces on districts.ProvinceId equals province.ProvinceId
                        into provincelist
                    from provinces in provincelist.DefaultIfEmpty()
                    where visits.FacilityId == filterdata.FacilityId && visits.ServicePointId ==
                                                                     filterdata.ServicePointId
                                                                     && provinces.ProvinceId == filterdata.ProvinceId &&
        ...
        public List<SMSRecords> GetAppointementsDetailsForSMS()
        {
            List<SMSRecords> smsRecords = (from appointment in _DigitalEdgeContext.Appointments
                ...
                join serviceType in _DigitalEdgeContext.ServiceTypes on appointment.ServiceTypeId equals serviceType
                    .ServiceTypeId into serviceTypesDetails
                from serviceTypes in serviceTypesDetails.DefaultIfEmpty()
                where (appointment.AppointmentDate >= DateTime.UtcNow &&
                       appointment.AppointmentDate < DateTime.UtcNow.Date.AddDays(8)) ||
                      (visits.NextAppointmentDate >= DateTime.UtcNow &&
                       visits.NextAppointmentDate < DateTime.UtcNow.Date.AddDays(8))
        ...
        public List<SMSRecords> SmsRecords(long id)
        {
            List<SMSRecords> smsRecords = (from appointment in _DigitalEdgeContext.Appointments
                ...
                from serviceTypes in serviceTypesDetails.DefaultIfEmpty()*/
                where ((appointment.AppointmentDate >= DateTime.UtcNow ||
                        visits.NextAppointmentDate >= DateTime.UtcNow) && appointment.AppointmentId == id)
                select new SMSRecords
                {
                    ClientId = clients.ClientId,
                    FirstName = clients.FirstName,
        ...
        public List<SMSRecords> SmsRecordsForVisits(long id)
        {
            List<SMSRecords> smsRecords = (from appointment in _DigitalEdgeContext.Appointments
                ...
                where ((appointment.AppointmentDate >= DateTime.UtcNow ||
                        visits.NextAppointmentDate >= DateTime.UtcNow) && visits.VisitId == id)
                select new SMSRecords
                {
                    ClientId = clients.ClientId,
                    FirstName = clients.FirstName,
        ...
        {
            List<ServicePoint> servicePointDetails =
                this._servicePointRepository.Get().Where(x => x.FacilityId == id).ToList();
        ...
        {
            MessageTemplate messageTemplateDetails = this._messageTemplateRepository.Get().Where(x =>
                    x.Type == messageTemplate.Type && x.Language == messageTemplate.Language && x.Status == true)
                .FirstOrDefault();
        ...
        {
            _DigitalEdgeContext.Remove(
                _DigitalEdgeContext.ServicePoints.Single(a => a.ServicePointId == servicePointId));
            _DigitalEdgeContext.SaveChanges();
        }
        ...
        public Client GetClientById(long id)
        {
            var client = (from singleClient in _DigitalEdgeContext.Clients
                    ...
                    join clientStatus in _DigitalEdgeContext.ClientStatuses on singleClient.ClientStatusId equals
                        clientStatus.ClientStatusId into statuses
                    from clientStatuses in statuses.DefaultIfEmpty()
                    join facility in _DigitalEdgeContext.Facilities on singleClient.FacilityId equals facility
                        .FacilityId into facilities
        ...
        public List<FacilityModel> GetFacilities()
        {
            List<FacilityModel> facilities = (from facility in _DigitalEdgeContext.Facilities
                    join facilityType in _DigitalEdgeContext.FacilityTypes on facility.FacilityTypeId equals
                        facilityType.FacilityTypeId
                    join districtName in _DigitalEdgeContext.Districts on facility.DistrictId equals districtName
                        .DistrictId
        ...
        public Appointment GetAppointmentById(long id)
        {
            var appointment = (from appointments in _DigitalEdgeContext.Appointments
                    join client in _DigitalEdgeContext.Clients on appointments.ClientId equals client.ClientId into
                        clients
                    from clientAppointments in clients.DefaultIfEmpty()
                    join facility in _DigitalEdgeContext.Facilities on appointments.FacilityId equals facility
                        .FacilityId into facilities
                    from appointmentInFacility in facilities.DefaultIfEmpty()
                    join serviceType in _DigitalEdgeContext.ServiceTypes on appointments.ServiceTypeId equals
                        serviceType.ServiceTypeId into serviceType
        ...
        public Facility GetFacilityById(long id)
        {
            var facility = (from facilities in _DigitalEdgeContext.Facilities
                    join facilityTypes in _DigitalEdgeContext.FacilityTypes on facilities.FacilityTypeId equals
                        facilityTypes.FacilityTypeId into buildings
                    from building in buildings.DefaultIfEmpty()
                    join distirct in _DigitalEdgeContext.Districts on facilities.DistrictId equals distirct.DistrictId
                        into facilityDistrict
        ...
        public Visit GetVisitById(long id)
        {
            var visit = (from visits in _DigitalEdgeContext.Visits
                    ...
                    join appointment in _DigitalEdgeContext.Appointments on visits.AppointmentId equals appointment
                        .AppointmentId into dbAppointments
                    from clientAppointment in dbAppointments.DefaultIfEmpty()
                    join servicePoint in _DigitalEdgeContext.ServicePoints on visits.ServicePointId equals servicePoint
                        .ServicePointId into dbServicePoints
        ...
        public List<FacilityModel> GetFacilitiesInDistrict(long id)
        {
            List<FacilityModel> facilitiesInDistrict = (from facility in _DigitalEdgeContext.Facilities
                ...
                join facilityType in _DigitalEdgeContext.FacilityTypes on facility.FacilityTypeId equals facilityType
                    .FacilityTypeId
        ...
        public IEnumerable<SearchModel> SearchClient(string searchTerm)
        {
            IEnumerable<SearchModel> query = (from client in _DigitalEdgeContext.Clients
                join facility in _DigitalEdgeContext.Facilities on client.FacilityId equals facility.FacilityId into
                    appointmentFacility
                from f in appointmentFacility.DefaultIfEmpty()
                where (client.FirstName.Contains(searchTerm) || client.LastName.Contains(searchTerm) ||
                       client.ArtNo.Contains(searchTerm) || client.ClientPhoneNo.Contains(searchTerm))
        ...
        public IEnumerable<SearchModel> SearchAppointment(string searchTerm)
        {
            IEnumerable<SearchModel> query = (from appointment in _DigitalEdgeContext.Appointments
                ...
                join facility in _DigitalEdgeContext.Facilities on appointment.FacilityId equals facility.FacilityId
                    into appointmentFacility
                from f in appointmentFacility.DefaultIfEmpty()
                join serviceType in _DigitalEdgeContext.ServiceTypes on appointment.ServiceTypeId equals serviceType
                    .ServiceTypeId into appointmentServiceType
                from st in appointmentServiceType.DefaultIfEmpty()
                where (clientAppointment.FirstName.Contains(searchTerm) ||
                       clientAppointment.LastName.Contains(searchTerm) ||
                       clientAppointment.ArtNo.Contains(searchTerm) ||
                       clientAppointment.ClientPhoneNo.Contains(searchTerm))
                select new SearchModel
                {
